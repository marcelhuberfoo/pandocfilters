    ...
    if key == 'Image':
        ...
        if mimet == 'image/svg+xml' and option:
            ...
            with urlopener.urlopen(src) as urlhandle:
                utime = urlhandle.headers.get(
                    'date', urlhandle.headers.get('last-modified'))
                utime = datetime.strptime(
                    utime, '%a, %d %b %Y %H:%M:%S %Z').timestamp()
                content = urlhandle.read()
            base_name = sha1(content.decode()
            ...
            if mtime < utime:
                import shlex
                cmd_line = shlex.split('rsvg-convert --format=' + option +
                                       ' --output ' + eps_name + ' /dev/stdin')
                sys.stderr.write("Running %s\n" % " ".join(cmd_line))
                runCommand(cmd_line,
                           logpath=os.getcwd(),
                           filename=os.path.basename(eps_name),
                           stdincontent=content)
            return Image([_id, _classes, _keyvallists], [alt],
                         [eps_name, title])
    return None
